...
import type { Toast, ToastType, DefaultToastOptions } from './types';

const TOAST_LIMIT = 20;

export const toasts = writable<Toast[]>([]);
export const pausedAt = writable<number | null>(null);
...
export function update(toast: Partial<Toast> & { id: string }) {
	if (toast.id) {
...
export function remove(toastId?: string) {
	toasts.update(($toasts) => {
...
export function useToasterStore(toastOptions: DefaultToastOptions = {}) {
	const mergedToasts = derived(toasts, ($toasts) =>
		$toasts.map((t) => ({
			...toastOptions,
			...(toastOptions[t.type] || {}),
			...t,
